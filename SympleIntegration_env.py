...
class IntegrateEnv(gym.Env):
    ...
    #     integrator = self.settings['Integration']['order'][index]
    ...
    def reset(self, options = None):
        ...
        self.gravity.parameters.integrator = self.settings['Integration']['order'][0]
        self.gravity.parameters.timestep = 1e-4 | self.units_time
    ...
    def step(self, action):
        ...
        t_step = self.settings['Integration']['t_step'][1] # TODO: modify to vary this as well

        t0_step = time.time()

        # Apply action
        current_int = self.settings['Integration']['order'][action]
        if current_int != self.previous_int: # if it's different, change integrator
            # self.gravity.stop()
            # self.gravity = self._get_integrator(action)()
            # self.gravity.particles.add_particles(self.particles) #TODO: needed again?
    ...
    def plot_orbit(self):
        ...
        for i in range(n_bodies):
            plt.plot(state[:, i, 2], state[:, i, 3], label = self.names[i])
        plt.legend()
        plt.show()
...
def evaluate_many_symple_cases(values):
    ...
    for i in range(cases):
        ax[0].plot(x_axis, E_E[:, i], color = colors[i], label = labels[i])
        ax[1].plot(x_axis, E_M[:, i], color = colors[i], label = labels[i])
...
steps = 200 # large value just in case
value1 = [0]*steps
value2 = [1]*steps
value3 = [2]*steps
value4 = [3]*steps
value5 = [4]*steps
value6 = np.random.randint(0, 5, size = steps)
