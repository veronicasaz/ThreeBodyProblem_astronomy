...
def plot_trajectory(settings, namefile = ""):
    state = np.load(settings['Integration']['savefile'] +'_state' + namefile + '.npy')
    cons = np.load(settings['Integration']['savefile'] +'_cons' + namefile + '.npy')
    t_comp = np.load(settings['Integration']['savefile'] +'_tcomp' + namefile + '.npy')

    # Divide into different actions found
    index_0 = np.where(cons[:,0] == 0)[0]
    index_1 = np.where(cons[:,0] == 1)[0]

    markers = ['o', 'x', '.', 's', '^', 'x', 'o'] # representing each action
    names = ['1','2','3','4','5','6','7']
